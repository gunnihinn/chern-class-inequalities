...
Before we do so we remark for our amusement an expression for the adjoint
of the Lefschetz operator that must be known but I had not seen before.
...
\begin{coro}
    ...
    $\Lambda = * L *$
    \hfil
    \phantom{\bf Corollary 1.7}
\end{coro}

\begin{proof}
It is enough to prove the identity for $(p,q)$--forms $u = L\^{p-j} u_j$,
where $u_j$ is a $(j,j+q-p)$--form and $p \leq q$. By setting $m = 2j+q-p$
and applying Proposition~\ref{16} twice we get
\begin{align*}
    *L*u &= *L (-1)^{m(m+1)/2} L\^{n-j-q} u_j \\
    &= * (-1)^{m(m+1)/2} \tbinom{n-j-q+1}{1} L\^{n-j-q+1} u_j \\
    &= \tbinom{n-j-q+1}{1} L\^{p-j-1} u_j = \Lambda u,
\end{align*}
where the last equality holds by Proposition~\ref{prop:morphism}.
\end{proof}
