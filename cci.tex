...
space $V$. Then
%% So the formula be valid for u with p > q we need to sum further up.
%% Figure out to what.
$$
    \lvert u \rvert^2 \omega\^n
    = \sum_{l=0}^p i^{(p-q)^2}(-1)^{q+l} b_l \, 
    (\Lambda\^ l u \wedge \Lambda\^ l \overline u)
...
$$
u^2 \wedge \omega^{n-p-q}
= \sum_{j=0}^p u_j^2 \wedge \omega^{n-2j-q+p}.
$$
\end{prop}
...
\begin{gather}
  ...
  \\
  L\^{n-p-q+2l}( \Lambda\^ l u \wedge \Lambda\^ l \overline u)
  \kern 193pt
  \\
  \label{18:2}
  \kern 70pt
...
That gives
\begin{align*}
L\^{n-p-q} u \wedge \overline u
&= \sum_{j=0}^p \frac{1}{(n-p-q)!}
\Bigl( \frac{u_j\wedge \overline u_j}{(p-j)!^2} \Bigr) 
\wedge \omega^{n-2j-q+p}\\
&= \sum_{j=0}^p \frac{1}{(n-p-q)!}\frac{(n-2j-q+p)!}{(p-j)!^2} 
u_j \wedge \overline u_j \wedge \omega\^{n-2j-q+p}\\
&= \sum_{j=0}^p \tbinom{n-j-q}{p-j} \tbinom{n-2j-q+p}{p-j}
u_j \wedge \overline u_j \wedge \omega\^{n-2j-q+p}
\\
&= \sum_{j=0}^p i^{p-q}(-1)^{\binom{2j+q-p+1}{2}} 
\tbinom{n-j-q}{p-j} \tbinom{n-2j-q+p}{p-j}
\lvert u_j \rvert^2 \omega\^{n}
\\
&= \sum_{j=0}^p i^{(q-p)^2}(-1)^j
\tbinom{n-j-q}{p-j} \tbinom{n-2j-q+p}{p-j}
\lvert u_j \rvert^2 \omega\^{n}.
\end{align*}
To get the general result, we apply this to the $(p-l,q-l)$--form
$\Lambda\^ l u$. That gives (by letting $p \mapsto p - l$, $q \mapsto q - l$)
\begin{align*}
  &L\^{n-p-q+l)} (\Lambda\^ l u \wedge \Lambda\^l \overline u)   
  \\
  &\qquad\quad
  = \sum_{j=0}^{p-l} 
  i^{(q-p)^2} (-1)^j
...
(\Lambda\^ l u \wedge \Lambda\^ l \overline u) \mid l = 0,\ldots,p),
\\
...
\begin{proof}[Proof of Theorem~\ref{thm:norm}]
    If $u$ is a $(p,q)$--form with $p \leq q$ then we write
    $$
    \lvert u \rvert^2 \omega\^n
    = \sum_{l=0}^p i^{(p-q)^2}(-1)^{q+l} b_l(p,n) \, 
    (\Lambda\^ l u \wedge \Lambda\^ l \overline u)
    \wedge \omega\^{n-p-q+2l},
    $$
...
i^{(p+q)^2}(-1)^q u \wedge \overline u \wedge \omega\^{n-2}$. Since
$i^{(p+q)^2} = i^{(p-q)^2}$ this proves the base step.
...
\begin{align*}
\lvert u \rvert^2 \omega\^n 
&= \sum_{l=0}^p i^{(p-q)^2}(-1)^{l+p} b_l(p,n) \, 
(\Lambda\^ l u \wedge \Lambda\^ l \overline u)
\wedge \omega\^{n-2(p-l)}
\\
&= \sum_{l=0}^p i^{(p-q)^2}(-1)^{l+p} b_l(p,n) \, 
...
\tbinom{n-p+l}{l}
\\
&= i^{(p-q)^2}\Bigl(\tbinom{n}{p} b_p(p,n)
...
= \sum_{l=0}^{p} 
i^{(p-q)^2}(-1)^{q+l} b_l \, 
(\Lambda\^l u) \wedge (\Lambda\^l \overline v) \wedge \omega\^{n-p-q+2l}.
$$
\end{coro}
